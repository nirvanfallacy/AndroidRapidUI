...
public class CustomEventHandlerInfo {
	private Method adder;
	private Class<?> listenerType;
	
	protected CustomEventHandlerInfo(Method adder, Class<?> listenerType) {
		this.adder = adder;
	...
	public static CustomEventHandlerInfo create(Object target, String category) {
		...
		while (targetType != null && !targetType.equals(Object.class)) {
			for (Method method2: targetType.getDeclaredMethods()) {
				...
				if (method2Name.equals(setterName) && paramTypes.length == 1) {
					return new CustomEventHandlerInfo(method2, paramTypes[0]);
		...
		if (adder == null || remover == null ||
			...
			return new UnregisterableCustomEventHandlerInfo(adder, remover, listenerTypeAdder);
		}
	}
}
